...
class SetCommandedPowerDialog(QDialog):
    """Dialog to set the commanded power."""
    ...
    def validate_and_accept(self):
        ...
        power = self.get_commanded_power()
        if power is not None:
            self.accept()
        else:
            QMessageBox.warning(self, "Invalid Input", "Please enter a valid non-negative number for the commanded power.")

    def get_commanded_power(self):
        """Get the commanded power from the input field."""
        try:
