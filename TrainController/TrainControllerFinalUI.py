...
class TrainControllerFinalUI(QWidget):
    def __init__(self, communicator: Communicate):
        ...
        self.speed_limit = 31.07    # Speed Limit in mph (50km/hr)
        ...
        self.timer.timeout.connect(lambda: self.handle_current_velocity(self.current_velocity))
        ...
        # Displaying the commanded authority in UI
        ...
        self.setpoint_box.addWidget(setpoint_label, alignment=Qt.AlignmentFlag.AlignCenter)  # Add to setpoint_box with center alignment
        ...
        self.all_failures = QHBoxLayout()
        self.all_failures.setSpacing(5)  # Set the spacing to 5 pixels
        
        self.all_failures.addLayout(self.train_engine_failure)
        self.all_failures.addLayout(self.brake_failure)
        self.all_failures.addLayout(self.signal_pickup_failure)

        # Add the vertical box layout to the main grid at position (9, 1)
        main_grid.addLayout(self.all_failures, 8, 1)
        ...
        # Create a vertical box layout for the current temperature section
        ...
        # Set the current temperature edit box to be non-editable
    ...
    def set_automatic_mode(self):
        ...
        # Set current speed to commanded speed
    ...
    def update_power_command(self):
        ...
        if (self.desired_velocity / 2.237) > self.speed_limit:
            self.desired_velocity = self.speed_limit
        else:
            self.desired_velocity = self.desired_velocity / 2.237
            
        # Print desired and current speed
        print(f"Desired Speed: {self.desired_velocity / 2.237:.2f} m/s, Current Speed: {self.current_velocity:.2f} m/s")
    ...
    def update_setpoint_speed_calculations(self):  
        ...
        self.desired_velocity = float(self.setpoint_speed_edit.text()) / 2.237
...
if __name__ == "__main__":
    ...
    ui = TrainControllerFinalUI(communicator)
    ui.show()
    sys.exit(app.exec())
