...
class CTCWaysideControllerComm(QObject):
    ...
    block_maintenance_signal = pyqtSignal(list)   # Bool for every block
    switch_signal = pyqtSignal(list)              # List of switch states

    
    # Wayside -> CTC
    block_occupancy_signal = pyqtSignal(list)     # Bool for every block